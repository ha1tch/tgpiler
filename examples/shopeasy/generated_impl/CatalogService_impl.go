...
func (r *CatalogServiceRepositorySQL) GetCategoryBySlug(ctx context.Context, req *GetCategoryBySlugRequest) (*GetCategoryBySlugResponse, error) {
	...
	var result CategoryBySlugResponse
	err := row.Scan()
	if err != nil {
		if err == sql.ErrNoRows {
...
func (r *CatalogServiceRepositorySQL) CreateProduct(ctx context.Context, req *CreateProductRequest) (*CreateProductResponse, error) {
	...
	row := r.db.QueryRowContext(ctx, query, req.SKU, req.Name, req.Slug, req.Description, nil, req.CategoryID, req.TrackInventory, req.InitialStock)
	
	var result CreateProductResponse
	err := row.Scan()
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, fmt.Errorf("CreateProduct: not found")
		}
